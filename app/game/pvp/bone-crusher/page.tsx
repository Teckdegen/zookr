...
const MOVES = { skull: '💀', shield: '🛡️', axe: '⚔️' } as const
...
export default function BoneCrusherPage() {
  ...
  if (result && gameData) {
    const gd = result.game_data as { rounds: { p1: Move; p2: Move; winner: string }[]; winner: string }
      ...
      <AppLayout active="play" fullBleed>
        <div className="min-h-screen bg-[#0A0806] flex flex-col items-center justify-center gap-6 px-6">
          ...
          <div className="w-full max-w-xs space-y-2">
            {gd.rounds.map((r, i) => {
              const myMove = isP1 ? r.p1 : r.p2
              const theirMove = isP1 ? r.p2 : r.p1
              const iWonRound = (isP1 && r.winner === 'p1') || (!isP1 && r.winner === 'p2')
              return (
                <div key={i} className={`flex justify-between items-center px-4 py-2 border ${iWonRound ? 'border-[#D4BF9A]/40' : r.winner === 'tie' ? 'border-[#2E2618]' : 'border-[#DC143C]/40'}`}>
                  <span className="font-mono text-[9px] text-[#7A6E58]">Round {i + 1}</span>
                  <span className="text-xl">{MOVES[myMove]}</span>
                  <span className="font-mono text-[9px] text-[#7A6E58]">vs</span>
                  <span className="text-xl">{MOVES[theirMove]}</span>
                  <span className="font-mono text-[9px]">{iWonRound ? '✓' : r.winner === 'tie' ? '=' : '✗'}</span>
                </div>
              )
            })}
          </div>
    ...
    return (
      <AppLayout active="play" fullBleed>
        <div className="min-h-screen bg-[#0A0806] flex flex-col items-center justify-center gap-8 px-6">
          ...
          <h2 className="font-serif text-3xl text-[#D4BF9A]">BONE CRUSHER</h2>
          <div className="text-center space-y-2">
            <p className="font-mono text-[10px] text-[#7A6E58]">💀 Skull crushes Shield</p>
            <p className="font-mono text-[10px] text-[#7A6E58]">🛡️ Shield blocks Axe</p>
            <p className="font-mono text-[10px] text-[#7A6E58]">⚔️ Axe cuts Skull</p>
          </div>
          <p className="font-mono text-[9px] text-[#7A6E58] text-center">Server picks all moves. Best of 5 wins.</p>
    ...
    <AppLayout active="play" fullBleed>
      <div className="flex flex-col min-h-[calc(100vh-56px)]">
        <div className="px-4 md:px-6 py-3 border-b border-[#2E2618] bg-[#1E1B14] flex items-center gap-3">
          ...
          <p className="font-mono text-[9px] text-[#7A6E58]">Skull, Shield, Axe</p>
        </div>
