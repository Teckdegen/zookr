...
export async function POST(req: NextRequest) {
  ...
  await supabase.from('room_round_entries').insert({
    round_id, user_id, pick: pick.toString(), result: 'pending',
  })

  const { data: round } = await supabase.from('room_rounds').select('*').eq('id', round_id).single()
  const { data: entries } = await supabase.from('room_round_entries').select('*').eq('round_id', round_id)
  ...
  if (round.game_type === 'flash_price') {
    const addr = token === 'UDEAD' ? UDEAD_ADDRESS : DEAD_ADDRESS
    const priceData = await fetchTokenPrice(addr)
    const realPrice = priceData?.priceUsd ?? 0
    let closestDiff = Infinity
    for (const entry of entries) {
      const diff = Math.abs(parseFloat(entry.pick) - realPrice)
      if (diff < closestDiff) { closestDiff = diff; winnerId = entry.user_id; winnerPick = entry.pick }
    }
  } else {
    let bestRoll = -1
    for (const entry of entries) {
      const val = parseInt(entry.pick)
      if (val > bestRoll) { bestRoll = val; winnerId = entry.user_id; winnerPick = entry.pick }
    }
  }

  if (!winnerId) return NextResponse.json({ waiting: true })

  const { data: winnerUser } = await supabase.from('users').select('username').eq('id', winnerId).single()
  const winnerUsername = winnerUser?.username ?? 'Unknown'
  const payout = round.winner_payout
  const usedToken = token === 'UDEAD' ? 'UDEAD' : 'DEAD'

  await supabase.from('room_rounds').update({ status: 'complete', winner_id: winnerId }).eq('id', round_id)

  for (const entry of entries) {
    await supabase.from('room_round_entries').update({ result: entry.user_id === winnerId ? 'win' : 'loss' }).eq('id', entry.id)
  ...
  const { data: roomData } = await supabase.from('room_rounds').select('room_id').eq('id', round_id).single()
  if (roomData) {
    await supabase.channel(`room:${roomData.room_id}`).send({
      type: 'broadcast', event: 'round_result',
      payload: { winner_id: winnerId, winner_username: winnerUsername, winner_pick: winnerPick, payout },
    })
  }
