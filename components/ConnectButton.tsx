    ...
    <>
      ...
      {open && (
        <WalletModal
          ...
          onConnect={(connector) => { connect({ connector }); setOpen(false) }}
          onClose={() => setOpen(false)}
        />
      )}
    </>
...
function WalletModal({ connectors, onConnect, onClose }: {
  connectors: readonly ReturnType<typeof useConnect>['connectors'][number][]
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center" onClick={onClose}>
      ...
      >
        ...
        <button
          onClick={onClose}
          className="mt-4 font-mono text-[9px] text-[#7A6E58] tracking-widest hover:text-[#D4BF9A] transition-colors"
        >
          Cancel
        </button>
      </div>
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center" onClick={onClose}>
      ...
      >
        ...
        <button onClick={onDisconnect} className="btn-blood w-full py-3 text-[11px]">
          Disconnect
        </button>
        <button onClick={onClose} className="mt-3 font-mono text-[9px] text-[#7A6E58] tracking-widest hover:text-[#D4BF9A] transition-colors w-full text-center">
          Close
        </button>
      </div>
    </div>
